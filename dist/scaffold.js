...
export async function scaffold(a) {
    ...
    if (a.notifications === 'email') {
        await step('Add Notifications stub', async () => {
            await renderAndCopyDir(TPL('extras/notifications'), path.join(dest, 'tools', 'notifications'), a);
        });
    }
    if (a.ci === 'github') {
    ...
    if (a.zephyr) {
        await step('Add Zephyr publish stub', async () => {
            await renderAndCopyDir(TPL('extras/publications'), path.join(dest, 'tools', 'publications'), a);
        });
    }
    ...
    const devDeps = {
        ...
        kolorist: '^1.8.0',
        'allure-playwright': a.reporter === 'allure' ? '^3.2.1' : undefined,
        // Add the command-line if user chose Allure
        'allure-commandline': a.reporter === 'allure' ? '^2.34.1' : undefined,
        // Add Monocart only when chosen
        'monocart-reporter': a.reporter === 'monocart' ? '^2.9.18' : undefined,
